...
{{ site.product_formal_name }} has activated Swagger annotations for its embedded APIs. There are APIs that are hosted within the {{ site.product_mini_name }} server, which we call workspace master APIs for managing workspaces. And there are APIs that are hosted within each workspace, launched and hosted by a workspace agent that is injected into each workspace machine when it boots.
...
#### cURL - Create a project on workspace
```shell  
...
#### cURL - Execute a command and get output
```shell  
# Get the machine Id
