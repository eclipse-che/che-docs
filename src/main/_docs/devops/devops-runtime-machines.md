...
Information on each machine can be viewed in the in the `Dashboard`. Information on each machine in a workspace can be viewed after clicking on the workspace name, selecting the `Machines` tab. Machine information includes source and ram. Other machine information: [agents]({{base}}{{site.links["devops-ws-agents"]}}), exposed ports, and environment variables on the appropriate tabs: `Agents`, `Servers`, `Env Variables`. All of these configuration items can be changed on a stopped workspace/machine. If the workspace/machine is running, the workspace will be stopped automatically then running again after the save configuration. Changes made to the runtime configuration will only affect the workspace and will not be saved to original stack to be create other workspaces from.
