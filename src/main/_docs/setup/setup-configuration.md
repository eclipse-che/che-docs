...
Che-server comes with `logstash-json-encoder` which allows you to log into a json format for Logstash or any other log consumers.
...
This appender will log in json format to a rolling `/logs/logs/catalina.log.json` file.

Others `logback-logstash-encoder` appenders can be added as well. See <https://github.com/logstash/logstash-logback-encoder#usage>.
