...
function navigate(direction) {
	if(direction == 'up' && currentSelection != -1) {
		...
		if(currentSelection < $("#results-container li").size() -1) {
			currentSelection++;
		}
	}
...
function setSelected(menuitem) {
	var title = $("#results-container li a").eq(menuitem).attr('title');
	$("#results-container li a").removeClass("search_hover");
	$("#results-container li a").eq(menuitem).addClass("search_hover");
	currentUrl = $("#results-container li a").eq(menuitem).attr("href");
}
