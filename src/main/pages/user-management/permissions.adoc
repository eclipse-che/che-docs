...
On Start, Che will grant this permission (along with the rest System permissions) to the admin user, with default name `admin`.
This can be reconfigured with defining `CHE_SYSTEM_ADMIN__NAME` property, that will point to the desired name of admin user.
Permissions will be added to user as soon as he is persisted in Che Database (during his first login).
