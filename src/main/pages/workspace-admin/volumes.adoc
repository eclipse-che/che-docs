...
In order for your data to persist for a local Maven repository, the `node_modules/` directory, Ruby jems, or the `authorized_keys` file for SSH connections, your workspace will need additional volumes. You can add additional volumes for your workspace machines. Each machine can add as many volumes as the underlying infrastructure can support. OpenShift may impose a limit on the number of volumes.
...
* The name and path cannot contain special characters, including `-` and `_`.
