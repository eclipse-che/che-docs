...
Binding ports to the host system is not supported.  You must ensure that each container does not use already assigned host ports.  Many developers can be running the same Compose file on the same Che server host at the same time. You can add link:servers.html[servers] to the machines.
...
1.  Che allows user to create only pods and services.
2.  If you have an exception from Kubernetes, other object kinds such as PVCs and routes are ignored or a workspace fails to starts.
