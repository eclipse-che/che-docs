...
1.  Use a non-terminating `CMD` or `ENTRYPOINT`.  You can use `tail -f /dev/null` as one of the main processes if you are using a custom image.
...
2.  Large images can take a long time to build.  If you have a long Dockerfile, it is recommended to build that image locally, push it to DockerHub and then use the pushed image as a Docker-formatted container image recipe type. The start timeout for a workspace is five minutes.

[id="running-multi-container-workspaces-using-compose-files"]
== Running multi-container workspaces using Compose files

You can run multi-container workspaces using Compose files on Docker.  There is some unsupported syntax: `Local "build.context" and "build.dockerfile"`.
...
The hostname is not supported.  The machine’s name is used for the hostname. You can use `links` aliases syntax to add additional hostnames to a machine.

*Binding ports*

Binding ports to the host system is not supported to ensure that each container does not use already assigned host ports. You can add link:servers.html[servers] to the machines.
