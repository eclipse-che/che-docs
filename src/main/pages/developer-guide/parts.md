...
Sample plugins: [IDE](https://github.com/che-samples/che-plugin-menu) and [IDE-Server Side](https://github.com/che-samples/che-ide-server-extension).
...
{% include image.html file="devel/parts.png" %}
...
Finally, we have to make our view available for other components, using dependency injection. This is done in `MyGinModule`, which can contain other bindings, too. Please see the section [Dependency Injection Basics][guice] for more details about this binding.
...
The following code example shows an action, which opens the "Hello World" part defined before. Please see the section [Actions][actions] for more details about the implementation of actions.
