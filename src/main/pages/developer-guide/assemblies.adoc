...
However, if you build a custom assembly and your plugin provides client side changes, it is root pom.xml of *assembly-ide-war* that dependencies should be added to. See: link:ide-extensions-gwt[IDE Extensions].
...
Now that you have got some knowledge about Che assemblies and own cloned a sample assembly, let’s take a closer look at Eclipse Che link:ide-extensions-gwt[client] and link:server-side-extensions[server-side] plugins.
