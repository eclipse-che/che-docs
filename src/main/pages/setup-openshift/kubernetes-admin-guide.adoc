...
Default workspace RAM limit, and RAM allocation request, can be configured with `CHE_WORKSPACE_DEFAULT__MEMORY__LIMIT__MB` and `CHE_WORKSPACE_DEFAULT__MEMORY__REQUEST__MB`  parameters appropriately passed to Che deployment.

Note, that those default values can be overriden when creating workspace, if specified by the user.
