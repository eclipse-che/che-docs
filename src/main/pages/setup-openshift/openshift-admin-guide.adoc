...
. Update Keycloak and Postgres deployments (optional):
...
To delete data or request the admininistrator to delete data, do this command with the user or adminstrator token:
...
To run Che Server in debug mode, set the following environment variable in Che deployment to true (default is false):
