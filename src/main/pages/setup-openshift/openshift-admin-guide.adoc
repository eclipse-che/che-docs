...
To change the access mode for workspace PVCs, pass the environment variable `CHE_INFRA_KUBERNETES_PVC_ACCESS_MODE=ReadWriteMany` to Che deployment either when initially deploying Che or through the Che deployment update.
...
* Workspaces that are started five to thirty seconds before the network traffic are switched to the new pod and may fallback to the stopped state. This happens because the bootstrappers use the Che server route URL for notifying the Che Server when bootstrapping is done. Since traffic is already switched to the new Che server, the old Che server cannot get the bootstrapper's report, and fails to start after the waiting timeout is reached. If the old Che server is killed before this timeout, the workspaces can be stuck in the `STARTING` state. The `terminationGracePeriodSeconds` parameter must define enough time to cover the workspace start timeout timeout, which is eight minutes plus some additional time. Typically, setting `terminationGracePeriodSeconds` to 540 sec is enough to cover all timeouts.
...
* Set the `CHE_INFRA_OPENSHIFT_PROJECT` environment variable to `NULL` to ensure a new distinct OpenShift namespace is created for every workspace that is started.
