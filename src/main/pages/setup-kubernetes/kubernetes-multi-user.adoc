...
----
+
* Install `tiller`, which is the Helm server, on your cluster.
...
If using the IP address, for example, when your corporate policy prevents you from using nip.io, you must also set `isHostBased` to `false`.
