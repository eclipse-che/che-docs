...
* Set the `CHE_INFRA_OPENSHIFT_OAUTH__IDENTITY__PROVIDER` variable to the alias of the OpenShift identity provider specified in step 1 of its link:#openshift-identity-provider-registration.html[registration in Keycloak]. The default value is `openshift-v3`.
