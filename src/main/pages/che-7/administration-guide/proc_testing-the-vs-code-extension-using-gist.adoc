...
To add a plug-in that is not in the default plug-in registry, build a custom plug-in registry. See link:{site-baseurl}che-7/building-and-running-a-custom-registry-image#building-a-custom-plug-in-registry_building-and-running-a-custom-registry-image[the documentation on building custom registries], or, alternatively, use GitHub and the gist service.
