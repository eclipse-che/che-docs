...
To add a plug-in if it is not in the default plug-in registry, you can build your custom plug-in registry. See link:{link-building-a-custom-plug-in-registry}[the documentation on building custom registries], or, alternatively, you can use GitHub and the gist service.
...
. Go to the link:https://gist.github.com/[gist] webpage and create a `README.md` file with the following description `Try Bracket Pair Colorizer extension in {prod}` and content `Example of VS Code extension`
link:https://marketplace.visualstudio.com/items?itemName=CoenraadS.bracket-pair-colorizer[Bracket Pair Colorizer] is a popular VS Code extension.

. Click btn:[Create secret gist].
+
image::workspaces/vscode-gist-setup.png[]

. Clone gist repository by using the URL from the navigation bar of the browser:
+
[subs="+quotes"]
----
...
----
<1> Directory name matched gist ID.
...
----
<1> This extension requires a basic Node.js runtime so in the YAML definition there is no need to add a custom runtime image.
<2> {{REPOSITORY}} is a macro for a pre-commit hook.

. Define memory limit and volumes:
+
[source,yaml,subs="+quotes"]
----
spec:
...
. Create `devfile.yaml` that references the `plugin.yaml` file:
+
[source,yaml,subs="+quotes"]
----
apiVersion: 1.0.0
  ...
  -
    ...
    actions:
      -
        ...
        workdir: ${CHE_PROJECTS_ROOT}/console-java-simple
----
<1> Any other devfile definition is accepted, important information from this devfile are the lines defining this external component
...
. Verify that you have 4 files in the current Git directory:
+
[subs="+quotes"]
----
$ ls -la
...
----
+
The hook replaces `{{REPOSITORY}}` macro and add the external raw link to the gist.
...
. Commit and push the files the files:
+
[subs="+quotes"]
----
# Add files
...
. Visit the gist website and verify that all links have the correct public URL and do not contain anymore `{{REPOSITORY}}` variable. You can reach the devfile by using the following commands:
+
[subs="+quotes"]
----
