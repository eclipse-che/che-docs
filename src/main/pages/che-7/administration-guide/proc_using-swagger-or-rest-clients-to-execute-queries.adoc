...
Access the {prod-short} Swagger interface at `http://che_host:8080/swagger`. The user must be signed in through Keycloak, so that the access token is included in the *Request* header.
