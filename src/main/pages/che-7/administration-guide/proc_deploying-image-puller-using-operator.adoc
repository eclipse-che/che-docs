...
First, create a {orch-namespace} in your cluster to host the image puller.  Our example will use the {orch-namespace} "image-puller".

Navigate to your OpenShift cluster's console, and select *Operators*.  Select *OperatorHub* and type "image puller" into the "Filter by keyword.." search bar.  Click the *Kubernetes Image Puller Operator*, click *Continue* and click *Install*.  At the *Installation Mode* selection, choose *A specific {orch-namespace} on the cluster*, and use the drop-down to find the {orch-namespace} you created to install the image puller.  Click *Subscribe*.  
...
Navigate to the *Workloads* and *Pods* menu in the {orch-namespace} that the image puller was installed, and you should see pods being created.
