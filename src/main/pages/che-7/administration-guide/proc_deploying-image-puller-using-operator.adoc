...
First, create a {orch-namespace} in your cluster to host the image puller. This document uses the {orch-namespace} "image-puller" as an example.
...
. Read the descriotion of the Operator. Click btn:[*Continue*] → btn:[*Install*].
...
. You will be redirected to a screen with a YAML editor with a `KubernetesImagePuller` Custom Resource. Make any modifications to the Custom resource and click btn:[*Create*].
