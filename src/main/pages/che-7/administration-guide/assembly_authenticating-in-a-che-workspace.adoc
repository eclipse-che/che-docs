...
Machine tokens avoid the need to pass {identity-provider} tokens to workspace containers (which can be insecure). Also, {identity-provider} tokens may have a relatively shorter lifetime and require periodic renewals or refreshes, which is difficult to manage and keep in sync with the same user session tokens on clients.
