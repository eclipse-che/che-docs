---
title: Customizing the devfile and plug-in registries
keywords:
tags: []
sidebar: che_7_docs
permalink: che-7/customizing-registries.html
...
The devfile and plug-in registries run in two separate pods and are deployed when the Che server is deployed (that is the default behavior of the Helm chart or the Che operator). The metadata of the plug-ins and devfile are versioned on GitHub and follow the Che server life cycle.
...
.  Option 1: Consists of building a custom image of the registries

.  Option 2: Consists of running the default images but modifying them at runtime

include::assembly_building-and-running-a-custom-registry-image.adoc[leveloffset=+1]

include::proc_including-the-plug-in-binaries-in-the-registry-image.adoc[leveloffset=+1]

include::assembly_editing-a-devfile-and-plug-in-at-runtime.adoc[leveloffset=+1]
