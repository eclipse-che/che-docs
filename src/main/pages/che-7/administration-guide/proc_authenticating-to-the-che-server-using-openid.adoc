...
. The service URL is `__<che.host>__:__<che.port>__/api/keycloak/settings`, and it is only available in the {prod-short} multi-user mode. The presence of the service in the URL confirms that the authentication is enabled in the current deployment.
+
Example output:
+
[source,json]
