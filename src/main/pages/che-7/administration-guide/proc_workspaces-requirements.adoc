...
|===
| `chePlugin` | `+CHE_WORKSPACE_SIDECAR_DEFAULT__CPU__LIMIT__CORES+`
| `cheEditor` | `+CHE_WORKSPACE_SIDECAR_DEFAULT__CPU__REQUEST__CORES+`
|===
...
Note that the `LimitRange` object of the Kubernetes namespace may specify defaults for CPU limits and requests set by cluster administrators. To prevent start errors due to resources overrun, limits on application or workspace levels must comply with those settings.
