...
<3> Resources Prometheus monitors. In the default configuration, there is a single job called `che`, which scrapes the time series data exposed by the {prod-short} server.
