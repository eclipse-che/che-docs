...
* {prod-short} is exposing metrics on port `8087`. See xref:enabling-and-exposing-che-metrics_{context}[Enabling and exposing {prod-id-short} metrics].
...
* Use the Prometheus console to query and view metrics. Metrics are available at:
+
[subs="+quotes"]
----
http://_<che-server-url>_:9090/graph
----
+
