...
The described method is to write a {prod-short}-Theia plug-in that uses the _languageserver-node_ library to connect a language server to the VS Code extension API. Note that it is also possible to directly connect a {prod-short}-Theia plug-in or a VS Code extension to the plug-in API. 

.Prerequisites
* Install the latest {prod-short}-Theia plug-in Yeoman generator. Follow the documentation about link:{site-baseurl}che-7/developing-che-theia-plug-ins[how to develop {prod-short}-Theia plug-in with Yeoman].
