[id="developing-{prod-id-short}-theia-plug-ins-using-che_{context}"]
= Developing a {prod-short}-Theia plug-in using {prod-short}

This section describes how to set up an environment for developing the {prod-short}-Theia plug-ins in {prod}.
...
----
+
.Building a {prod-short}-Theia plug-in using `yarn`
...
+
NOTE: If you get a warning that your browser prevented the opening of a new tab, click the btn:[Open] button in the dialog box. A new tab with another {prod-short}-Theia instance (with the developed plug-in loaded) is opened.
+
.Running a {prod-short}-Theia plug-in in Hosted mode
...
.. Go back to the *Main {prod-short}-Theia* window, and use the *Debug* panel as you need.
+
.Debugging a {prod-short}-Theia plug-in
