...
<3> `logo`: product logo for dark and light themes on the *Welcome* page (maximum height 80px) and in the *About* dialog (maximum height 100px). Use an image with a transparent background. Define a relative path, an absolute path, or a URL to an image.
