...
+
Example: The `FROM` directive should be similar to `FROM eclipse/che-theia-endpoint-runtime:next`. This is required because this base image contains tools for running the remote VS Code extension and communications between the sidecar and the {prod-short}-Theia editor, so that the VS Code extension does not have to know that it is a remote one.
