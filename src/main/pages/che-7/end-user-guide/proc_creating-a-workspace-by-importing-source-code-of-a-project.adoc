...
* A workspace with plug-ins related to your development environment on this instance of Eclipse Che xref:creating-a-workspace-from-user-dashboard_{context}[Creating a workspace from user dashboard].

There are two ways to do that *before* starting a workspace:

* xref:#importing-from-the-dashboard-into-an-existing-workspaces[Select a stack from the *Dashboard*, then change the devfile to include your project]
