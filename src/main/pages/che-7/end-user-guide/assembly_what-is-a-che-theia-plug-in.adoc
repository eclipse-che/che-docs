...
[id='what-is-a-{prod-id-short}-theia-plug-in']
= What is a {prod-short}-Theia plug-in

:context: what-is-a-che-theia-plug-in

A {prod-short}-Theia plug-in is an extension of the development environment isolated from the IDE. Plug-ins can be packaged as files or containers to provide their own dependencies.

Extending {prod-short}-Theia using plug-ins can enable the following capabilities:
...
== Features and benefits of {prod-short}-Theia plug-ins
...
| *Secure Loading*      | Plug-ins are loaded separately from the IDE. The IDE stays always in an usable state. | Plug-ins do not break the whole IDE if it has bugs. Handle network issue.
