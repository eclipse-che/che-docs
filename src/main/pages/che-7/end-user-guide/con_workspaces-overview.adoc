...
IMPORTANT: By default, it is possible to run only one workspace at a time. To change the default value, see link:{site-baseurl}che-7/advanced-configuration-options/#limits-for-the-workspaces-on-an-user[Limits for the workspaces of a user].
