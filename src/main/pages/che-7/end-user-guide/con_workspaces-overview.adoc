...
IMPORTANT: By default, it is possible to run only one workspace at a time. To change the default value, see: {link-limits-for-the-workspaces-of-an-user}.
...
* link:{site-baseurl}che-7/converting-a-che-6-workspace-to-a-che-7-devfile[Converting a {prod-short} {prod-prev-ver} workspace to a {prod-ver} devfile]
