...
[id='defining-custom-commands-for-{prod-id-short}-theia']
= Defining custom commands for {prod-short}-Theia

:context: defining-custom-commands-for-che-theia

The {prod-short}-Theia IDE allows users to define custom commands in a devfile that are then available when working in a workspace.
...
{prod-short} commands:: `theia:build`
+
