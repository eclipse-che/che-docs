...
Starting with {prod} {prod-ver}, Visual Studio Code (VS Code) extensions can be installed to extend the functionality of a {prod-short} workspace. VS Code extensions can run in the {prod-short}-Theia editor container, or they can be packaged in their own isolated and pre-configured containers with their prerequisites.
