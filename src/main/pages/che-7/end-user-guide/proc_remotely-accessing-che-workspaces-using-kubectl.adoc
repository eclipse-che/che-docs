...
[id="remotely-accessing-che-workspaces-using-the-kubernetes-command-line-tool_{context}"]
= Remotely accessing Che workspaces using the Kubernetes command-line tool
...
----
+
If the version is 1.5.0 or higher, proceed with the steps in this section.
// If not, should the user be upgrading to version 1.5.0 or higher? If yes, we must say so here.
