...
Since 7.15 version of {prod-short}, `target-container` annotation is deprecated, and another one `automount-workspace-secret`
with boolean values is introduced. Its purpose is to define global secret mounting behavior, with ability to override in devfile. So value of `true` enables automount into all workspace containers, while `false` disables mounting until
it is explicitly requested in devfile component using `automountWorkspaceSecrets:true` property.
+
[source,yaml]
----
apiVersion: v1
