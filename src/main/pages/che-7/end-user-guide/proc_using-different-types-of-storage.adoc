...
Since 7.18.0 Eclipse Che  added ability to shutdown Asynchronous Storage pod if it not used some time. To configure this behavior need to know about next configuration properties:

`che.infra.kubernetes.async.storage.shutdown_timeout_min` :  the timeout for shutdown Asynchronous Storage Pod after stopping last used workspace.

'che.infra.kubernetes.async.storage.shutdown_check_period_min': defines the period with which Asynchronous Storage Pod stopping ability will be performed.
