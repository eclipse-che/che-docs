...
In case of the multi-host strategy, you must set the che.infra.kubernetes.ingress.domain (or the `CHE_INFRA_KUBERNETES_INGRESS_DOMAIN` environment variable) configuration property to the domain name that will host workspace component subdomains.
