...
=== Overriding containers entrypoints
...
=== Overriding containers environment variables

You may need to provision/override entrypoints in kubernetes/openshift component then you're able to configure it in the following way
...
It's especially useful when you don't have access to edit referenced content, or you want to do it temporary.
Note that the specified environment variables will be provisioned into each init container and container inside all Pods, Deployments.
