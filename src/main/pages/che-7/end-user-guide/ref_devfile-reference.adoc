...
By default, volumes and PVCs specified in a devfile are bound to a host folder to persist data even after a container restart. Sometimes, it may be necessary to disable data persistence, such as when volume back end is slow, and it is needed to make workspace faster. To achieve it, the `persistVolumes` devfile attribute should be used. The default value is `true`, and in case of `false`, `emptyDir` volumes will be used for configured volumes and PVC.
