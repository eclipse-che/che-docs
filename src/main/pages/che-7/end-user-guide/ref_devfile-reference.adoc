...
The content can be provided either through the `reference` attribute, which points to the file with the component content.
...
`Dockerimage`, `chePlugin` and `cheEditor` type components can specify the custom volumes to be mounted on specific locations within the image. Note that the volume names are shared across all components and therefore this mechanism can also be used to share file systems between components.
...
Note, however, that the devfile RAM limit values have precedence, so the limit value in the devfile will override the value provided by the descriptor.
...
Note, however, that the devfile RAM request values have precedence, so the limit value in the devfile will override the value provided by the descriptor.
...
Note, however, that the devfile CPU limit values have precedence, so the limit value in the devfile will override the value provided by the descriptor.
...
Note, however, that the devfile CPU request values have precedence, so the limit value in the devfile will override the value provided by the descriptor.
...
* `CHE_WORKSPACE_NAMESPACE`: The Che namespace of the current workspace. This environment variable is the name of the user or organization that the workspace belongs to. Note that this is different from
