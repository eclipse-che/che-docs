...
<2> `sparseCheckoutDir` the template for the sparse checkout Git feature.

////
TODO: @mmorhun -- I don't understand this sentence. Could you please try to reword it?
...
.Example of the `clonePath` parameter
[source,yaml]
----
apiVersion: 1.0.0
metadata:
...
* Use the predefined variables in your own definitions.
====
...
Complex deployments can be described using Kubernetes or OpenShift resource lists that can be referenced in the devfile. This makea them a part of the workspace.
