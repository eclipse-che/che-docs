...
+
The trailing slash indicates that only directories with the given name (including their content) will be created.
...
* `unsecuredPaths`: A comma-separated list of the unsecured endpoint paths, staying as unsecured even if the `secure` attribute is set to `true`.
...
Each {prod-short}-specific command has an *action* attribute that is a command to execute and a *component* attribute that specifies the container for the command execution. The commands are run using the default shell in the container.
...
+
* `on` -- enables a notification that ask the user about the URL opening preferences
...
When an editor is not specified in a devfile, a default is provided. When no editor is needed, the `editorFree` attribute is applied. The default value is `false`, and it means that the devfile needs the default editor to be provisioned.
...
By default, volumes and PVCs specified in a devfile are bound to a host folder to persist data even after a container restart. Sometimes, it may be necessary to disable data persistence, such as when volume back end is slow, and it is needed to make the workspace faster. To achieve it, the `persistVolumes` devfile attribute must be changed. The default value is `true`, and in case of `false`, `emptyDir` volumes will be used for configured volumes and PVC.
