...
`airGapContainerRegistryHostname`: omit: An optional host name or URL to an alternate container registry to pull images from. This value overrides the container registry host name defined in all default container images involved in a {prod-short} deployment. This is particularly useful to install {prod-short} in an air-gapped environment.
...
`securityContextFsGroup,omitempty`: `1724`: FSGroup the {prod-short} Pod and Workspace Pods containers will run in.
`securityContextRunAsUser`: `1724`: ID of the user the {prod-short} Pod and Workspace Pods containers will run as.
