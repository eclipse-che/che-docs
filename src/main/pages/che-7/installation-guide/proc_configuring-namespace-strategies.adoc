...
[IMPORTANT]
====
Using the _One namespace per user_ or _One namespace for all workspaces_ strategy with the `common` PVC
strategy requires persistent volumes with `ReadWriteMany` access mode if more than 1 workspace needs to be running
concurrently. Because this might be tricky to set up reliably you may want to use a different PVC strategy or, in case
of _One namespace per user_, to limit the number of concurrently running workspaces per user to 1.

To enforce this limit, set:
