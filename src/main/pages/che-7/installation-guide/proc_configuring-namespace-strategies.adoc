...
To use the strategy, the `CHE_INFRA_KUBERNETES_NAMESPACE_DEFAULT` variable value must contain the `__<workspaceID>__` identifier. It can be used alone or combined with other identifiers or any string.
...
To use the strategy, the `CHE_INFRA_KUBERNETES_NAMESPACE_DEFAULT` variable value must contain one or more user identifiers. Currently supported identifiers are `__<username>__` and `__<userID>__`.
