...
* The link:https://github.com/che-incubator/chectl[`{prod-cli}`] command-line tool for managing a {prod-short} server and its development workspaces. See link:{site-baseurl}che-7/installing-the-chectl-management-tool/[Installing the `{prod-cli}` management tool].
