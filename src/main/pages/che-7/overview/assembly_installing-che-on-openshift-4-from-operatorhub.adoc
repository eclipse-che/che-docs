...
[NOTE]
====
It is possible to use the `{prod-cli}` utility script for deploying {prod-short} on {ocp} and {osd} versions {oc4-ver}. This method is considered unofficial and serves as a back-up installation method for situations where the installation method using OperatorHub is not available.

For information how to use the `{prod-cli}` utility script for deploying {prod-short} on OpenShift, see the
