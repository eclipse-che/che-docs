...
When installing {prod} in the `default` namespace, the proper security contexts and file permissions are not set appropriately, causing the installation to fail. It is recommended for users to avoid installation of {prod-short} in the `default` namespace and instead create a new namespace, for example, `{project-context}`, to contain the {prod-short} installation.
