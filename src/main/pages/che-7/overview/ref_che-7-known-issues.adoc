...
When {prod} in installed in non-tls (i.e. http) mode, and at the same time it is installed on distributed cluster, where the hostnames of Che server and workspace routes are different, latest Chrome versions may reject to open workspaces due to limitations in using `SameSite` and `Secure` policies of auth cookies set by JWTProxy. 
