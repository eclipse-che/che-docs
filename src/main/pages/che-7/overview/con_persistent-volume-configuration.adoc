...
When a user defines volumes for components in the devfile, all components that define the volume of the same name will be backed by the same directory in the PV as `<PV>/<workspaceid>/<originalpvcname>`. Each component can have this location mounted on a different path in its containers.
