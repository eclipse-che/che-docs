...
All workspaces inside a Kubernetes-native {orch-namespace} use the same Persistent Volume Claim (PVC) as the default data storage when storing data such as the following in their declared volumes: 
...
* Only one workspace in the same {orch-namespace} may be running. See link:{site-baseurl}che-7/configuring-namespace-strategies[Configuring {orch-namespace} strategies].
...
*  Subpaths of the mounted Physical persistent volumes that reference user-defined PVCs are prefixed with `_<workspace-ID>_` or `__<PVC-name>__`. This ensures that the same PV data structure is set up with different PVC strategies. For details, see xref:how-subpaths-are-used-in-pvcs_{context}[].
