...
The corresponding containers volume mounts link to a common volume, and sub-paths are prefixed with `'{workspaceID}/{originalPVCName}'` . For more details, see xref:how-the-subpaths-are-used_{context}[].
...
*  Subpaths of the mounted Physical persistent volumes that reference user-defined PVCs are prefixed with `{workspace id}/{PVC name}`. This ensures that the same PV data structure is set up with different PVC strategies. For details, see xref:how-the-subpaths-are-used_{context}[].
