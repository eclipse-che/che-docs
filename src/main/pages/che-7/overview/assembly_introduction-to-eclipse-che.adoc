...
* Discovering link:{site-baseurl}che-7/supported-platforms/[{prod} supported platforms and their supported installation methods].
...
* Learning more about {prod}: xref:what-is-{prod-id-short}[What is {prod}] and link:{site-baseurl}che-7/high-level-{prod-id-short}-architecture[Architecture overview].
