...
. The *Create CheCluster* page contains the configuration of the {prod-short} cluster to create. It is a YAML document describing the `CheCluster` Custom Resource to create. For an installation using the default configuration, keep the default values.
