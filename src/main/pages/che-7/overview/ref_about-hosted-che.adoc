...
Hosted {prod-short} is link:https://www.eclipse.org/che/[Eclipse {prod-short}] hosted by Red Hat.
...
Hosted {prod-short} has the following usage limits and terms of service:
...
+
NOTE: Hosted {prod-short} automatically deactivates accounts that have been inactive for more than 30 days. All existing workspaces in a deactivated account are lost. To start using Hosted {prod-short} again, a user must re-register.
