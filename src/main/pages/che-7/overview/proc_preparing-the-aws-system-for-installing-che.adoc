...
This document assumes that Eclipse Che must be configured to run at link:http://che.aws.my-ide.cloud[Che AWS Cloud].
...
. Configure the four DNS nameservers on the `my-ide.cloud` DNS. Note that if you are using your own DNS provider, updating the record takes a few hours.
+
...
. Add hosts on route 53 with this given hostname *link:https://console.aws.amazon.com/route53/home?region=eu-west-1#hosted-zones[]:*. Ensure that you include the colon (*:*) at the end of this URL.
