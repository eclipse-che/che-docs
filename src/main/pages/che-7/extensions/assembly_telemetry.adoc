...
Telemetry is the transparent and ethical collection of the usage data. By default, telemetry is not available in the {prod}, but there is an abstract API that allows enabling telemetry via the plug-in mechanism. This approach is used in the link:{site-baseurl}che-7/hosted-che/[Hosted {prod-short}] service where telemetry is enabled for every workspace.
...
* Programming languages and link:{site-baseurl}che-7/making-a-workspace-portable-using-a-devfile/#what-is-a-devfile_making-a-workspace-portable-using-a-devfile[devfiles] used in workspaces
...
Create a `meta.yaml` definition representing a Che Plugin that runs your custom back-end in a workspace Pod. For more information on `meta.yaml`, see link:{site-baseurl}che-7/what-is-a-{prod-id-short}-theia-plug-in/[What is a Che-Theia plug-in].
