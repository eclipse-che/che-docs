...
[NOTE]
====
Enable this option to allow users to connect their GitHub accounts from the {prod-short} Dashboard by using *User Preferences > Device Auth Tokens*. See xref:connecting-to-github-using-device-authorization.adoc[].
====
...
[NOTE]
====
When this ConfigMap is present, the *Connect to GitHub* button is displayed in *User Preferences > Device Auth Tokens*. Delete the ConfigMap to hide the button.
====
