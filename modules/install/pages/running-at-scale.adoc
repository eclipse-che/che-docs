...
:navtitle: Scalability limits and multi-cluster deployments
...
For more details on {ocp} tested object maximums, see Additional resources.
...
== How workspace size determines cluster capacity
...
== Why etcd is the primary scaling bottleneck
...
== How large Kubernetes objects strain etcd
...
== How DevWorkspace objects affect etcd storage
...
== Reduce etcd usage by disabling Copied CSVs
...
On large clusters with hundreds or thousands of namespaces, Copied CSVs consume an unsustainable amount of resources, including OLM memory, etcd storage, and network bandwidth. To remove the CSVs copied to every namespace, configure the `OLMConfig` object:
...
== Scale worker nodes to match workspace demand
...
== How Developer Sandbox runs Dev Spaces across clusters
...
== Route developers to the correct cluster automatically
...
== Map OpenShift groups to cluster URLs
