...
:navtitle: Install {prod-short} on the virtual {kubernetes} cluster
:description: To deploy {prod-short} when the host cluster version is incompatible or lacks external OIDC support, use a virtual {kubernetes} cluster (vCluster).
:keywords: installing-{prod-id-short}-on-the-virtual-kubernetes-cluster
:page-aliases: installing-che-on-the-virtual-kubernetes-cluster.adoc

[id="installing-{prod-id-short}-on-the-virtual-kubernetes-cluster"]
= Install {prod-short} on the virtual {kubernetes} cluster
...
* You have `vcluster` CLI installed. See link:https://www.vcluster.com/docs/getting-started/setup[Installing vCluster CLI].
...
----
DOMAIN_NAME=__<kubernetes_cluster_domain_name>__
----
...
[IMPORTANT]
====
If you use a registrar such as GoDaddy, add the following DNS record
