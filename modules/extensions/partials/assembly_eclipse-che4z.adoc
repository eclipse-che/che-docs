...
Powered by the open-source projects link:https://www.zowe.org/[Zowe] and link:https://www.eclipse.org/che/docs[Eclipse Che], Che4z offers an easy and streamlined on-boarding process to get new developers using the tools they need. Using container technology and stacks, {prod} brings the necessary technology to the task at hand.
