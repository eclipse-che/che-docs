...
* `controller.devfile.io/mount-to-devworkspace-include: '__<pattern>__'` mounts the PVC only to workspaces whose names match the pattern.
* `controller.devfile.io/mount-to-devworkspace-exclude: '__<pattern>__'` mounts the PVC to all workspaces except those whose names match.
