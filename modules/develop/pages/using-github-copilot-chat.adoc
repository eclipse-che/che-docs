...
[role="_abstract"]
Set up GitHub Copilot Chat in your {prod-short} workspace by installing the link:https://open-vsx.org/extension/redhat/devspaces-copilot-chat-integration/0.36.2[Dev Spaces Copilot Chat Integration] extension (version {copilot-ext-ver}) and authenticating with your GitHub account to get AI-assisted coding suggestions.
...
[IMPORTANT]
====
Use version {copilot-ext-ver} of the extension. Other versions might not be compatible with the current {prod-short} editor.
====

. Authenticate by using Device Authentication.
+
...
+
A notification is displayed with a device code. Click the link to open the GitHub device activation page in your browser, paste the code, and authorize access.
...
* Open the Copilot Chat panel, type a prompt such as "Explain this file", and verify that a response is displayed.
...
If you attempted to use Copilot Chat before completing Device Authentication, the extension might enter an error state. Typical symptoms include:

* Copilot Chat is connected but does not respond to prompts.
...
. Click the *Accounts* icon on the left activity bar, select your account, and click *Sign Out*. A confirmation dialog is displayed indicating the account has been used by the Dev Spaces Copilot Chat Integration. Click *Sign Out* to confirm.
