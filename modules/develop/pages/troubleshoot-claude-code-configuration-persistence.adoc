...
Match the symptoms below to identify the root cause and apply the corresponding solution.
...
* Use `controller.devfile.io/mount-to-devworkspace-include` or `controller.devfile.io/mount-to-devworkspace-exclude` annotations on the PVC to control which workspaces mount it. See xref:persist-claude-code-configuration.adoc#_filter_pvc_mounts_by_workspace_name[Filter PVC mounts by workspace name].
+
