...
.Prerequisites
* The public OpenVSX registry at link:https://open-vsx.org[open-vsx.org] is selected and accessible over the internet. See xref:configure:extensions-for-microsoft-visual-studio-code-open-source.adoc[Selecting an Open VSX registry instance].
+
[TIP]
====
To install recommended extensions in a restricted environment, consider the following options instead:

* xref:configure:configuring-the-open-vsx-registry-url.adoc[] to point to your OpenVSX registry.
