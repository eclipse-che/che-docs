...
To add a plug-in that is not in the default plug-in registry, build a custom plug-in registry. See xref:customizing-the-devfile-and-plug-in-registries.adoc[], or, alternatively, use GitHub and the gist service.
