...
* GitHub: link:https://docs.github.com/en/rest/users/users#get-a-user[Get a user]. Find the `id` value.
* GitLab: link:https://docs.gitlab.com/ee/api/users.html#for-normal-users[List users: For normal users], use the `username` filter: `/users?username=:username`. Find the `id` value.
* Bitbucket Server: link:https://developer.atlassian.com/server/bitbucket/rest/v802/api-group-users/#api-users-userslug-get[Get users {userSlug}]. Find the `account_id` value.
