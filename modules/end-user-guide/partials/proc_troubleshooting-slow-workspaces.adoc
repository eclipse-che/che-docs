...
_Role: Administrator_
+
Components of {prod-short} are OCI images. Setup {prod} in offline mode (air-gap scenario) to allow for reducing any extra download at runtime as everything needs to be present from the beginning. See xref:installation-guide:installing-che.adoc#installing-che-in-a-restricted-environment[].
