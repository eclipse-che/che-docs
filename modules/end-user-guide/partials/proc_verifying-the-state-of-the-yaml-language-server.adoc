...
The `node ********/server.js` running in the container indicates that the language server is running. If it is not running, the language server has not started inside the container. In this case, see link:#checking-important-logs_{context}[Checking important logs].
