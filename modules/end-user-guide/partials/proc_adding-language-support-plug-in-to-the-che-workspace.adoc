...
* A created workspace that is defined in this instance of {prod}. To define a workspace, see xref:configuring-a-workspace-with-dashboard.adoc[] and xref:creating-a-workspace-from-a-code-sample.adoc[].

 * The workspace must be in a **stopped** state. The steps to stop a workspace:
