...
. In the `components` section of the devfile , add the following lines to add the Java language plugin with Java 8 to the workspace:
+
...
. Click the btn:[Save] to save the changes.
. Restart the workspace.
+
When restarted, the workspace will include the new plug-in.
