...
To include a Che-Theia plug-in as embedded, define a URL to the plug-in binary file (the `.theia` archive) in the `meta.yaml` file. In the case of a VS Code extension, provide the extension ID from the Visual Studio Code  marketplace (see xref:administration-guide:using-a-visual-studio-code-extension-in-che.adoc[]).
