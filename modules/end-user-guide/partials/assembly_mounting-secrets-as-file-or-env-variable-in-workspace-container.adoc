...
The following section describes how to automatically mount the {platforms-name} secret in a workspace container and create permanent mount points for components such as:
...
A file:: Creates automatically mounted Maven settings that will be applied to every new workspace with Maven capabilities.

An environment variable:: Uses SSH key pairs and AWS authorization tokens for automatic authentication. 
+
...
include::partial$proc_overriding-a-secrets-mount-path-for-a-workspace-component.adoc[leveloffset=+2]
