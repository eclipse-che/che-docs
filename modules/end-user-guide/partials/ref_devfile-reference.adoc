...
Rather than using the editor or plug-in `id` to specify `cheEditor` or `chePlugin`, you can provide a direct link to the component descriptor (typically named `meta.yaml`) by using the `reference` field:
...
When starting a new server within a component, {prod-short} automatically detects this, and the UI offers to automatically expose this port as a `public` port. This is useful for debugging a web application, for example. It is impossible to do this for servers that automatically start with the container (for example, a database server). For such components, specify the endpoints explicitly.
