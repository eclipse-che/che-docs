...
An alternative way of specifying `cheEditor` or `chePlugin`, rather than a  use of the editor or plug-in `id` (and optionally an alternative registry), is to provide a direct link to the component descriptor (typically named `meta.yaml`) by using the `reference` field:
...
When starting a new server within a component, {prod-short} detects this automatically, and the UI offers to automatically expose this port as a `public` port. This is useful for debugging a web application, for example. It is impossible to do this for servers that start automatically with the container (for example, a database server). For such components, specify the endpoints explicitly.
