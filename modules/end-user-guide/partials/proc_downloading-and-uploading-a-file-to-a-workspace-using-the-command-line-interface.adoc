...
This procedure describes how to use the `{orch-cli} ` tool to download or upload files remotely from or to an {prod} workspace.
...
* The `{orch-cli} ` tool is available. See https://kubernetes.io/docs/tasks/tools/install-kubectl/[Installing `{orch-cli} `].
