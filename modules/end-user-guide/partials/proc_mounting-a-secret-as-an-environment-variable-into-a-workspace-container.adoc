...
metadata:
  ...
  annotations:
    ...
    che.eclipse.org/__<mykey>__env-name: __<FOO_ENV>__ <3>
    che.eclipse.org/__<otherkey>___env-name: __<OTHER_ENV>__ <4>
  labels:
    app.kubernetes.io/part-of: che.eclipse.org <5>
    app.kubernetes.io/component: workspace-secret <6>
data:
  __<mykey>__: __<myvalue>__ <7>
  __<otherkey>__: __<othervalue>__ <8>
----
<1> Enables automatic mounting into all workspace containers.
<2> Mounts the secret as a variable. 
<3> Provisions the `__<FOO_ENV>__` environment variable into all workspace containers using the value of __<mykey>__.
<4> Provisions the `__<OTHER_ENV>__` environment variable into all workspace containers using the value of __<otherkey>__.
<5> Matches the configured default label.
<6> Matches the configured default label.
<7> Provisions the `__<myvalue>__` data key value into all workspace containers.
<8> Provisions the `__<othervalue>__` data key value into all workspace containers.
