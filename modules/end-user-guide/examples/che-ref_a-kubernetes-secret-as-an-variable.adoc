...
// The corresponding procedure is underdeveloped. Parts of this example must end up in the procedure. A comprehensive rewrite is needed.
...
* In the next example, the environment variable `FOO_ENV` and the value `myvalue` will be provisioned into all workspace containers.
+
[source,yaml]
----
apiVersion: v1
