...
* You can override the mount path of the secret for specific workspace components by editing the devfile as follows: Declare an alias for the containers in the override path. Declare an additional volume. Match the volume's `name` and `containerPath` with the mount path of the secret. See the following example.
