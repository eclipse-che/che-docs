...
If the URL for starting a new workspace doesn't contain a parameter specifying the integrated development environment (IDE), then the workspace loads with the default IDE, Microsoft Visual Studio Code - Open Source.
...
NOTE: If the source-code Git repository to be cloned for workspaces contains a xref:defining-a-common-ide.adoc[`/.che/che-editor.yaml`] file, then the `che-editor=` parameter overrides it.
