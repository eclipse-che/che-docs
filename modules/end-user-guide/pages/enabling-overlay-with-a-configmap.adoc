...
This can be done manually after starting a workspace. If the administrator enabled `persistUserHome` in the `CheCluster` custom resource, the `storage.conf` file changes will persist after a workspace restart if the workspace does not use ephemeral storage. Another option is to build a new image with changes to `storage.conf` and use the new image for workspaces.
