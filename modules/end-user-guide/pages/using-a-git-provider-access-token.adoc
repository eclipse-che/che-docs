...
After applying the Secret, you can create new workspaces with clones of a private Git repository that is hosted on GitHub, GitLab, Bitbucket, or Microsoft Azure Repos.
...
<2> The Git provider name: `github` or `gitlab`, `bitbucket-server` or `azure-devops`.
<3> The Git provider URL.
<4> Your Git provider user ID.
<5> Your Git provider user organization only for `azure-devops` Git provider.
