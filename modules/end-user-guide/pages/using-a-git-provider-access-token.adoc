...
. Visit pass:a,c,q[{prod-url}]/api/user in the web browser and copy the `id` value from the response. This is your {prod-short} user ID.
...
[TIP]
====
* Visiting pass:a,c,q[{prod-url}]/api/kubernetes/namespace returns your {prod-short} user namespace as `name`.
