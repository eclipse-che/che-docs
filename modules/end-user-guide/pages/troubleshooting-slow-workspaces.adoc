...
+
{image-puller-name-short} is a tool that can cache images on each of {orch-name} nodes. As such, pre-pulling images can improve start times. See xref:configure:caching-images-for-faster-workspace-start.adoc[].
