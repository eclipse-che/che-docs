...
[role="_abstract"]
Prevent a workspace from stopping while long-running CLI tools such as `helm`, `odo`, or `sleep` are active in the terminal, by configuring a CLI Watcher with a `.noidle` file.
