...
** `{orch-cli}`. See link:https://kubernetes.io/docs/tasks/tools/#kubectl[Install Tools: kubectl].
endif::[]
ifeval::["{project-context}" != "che"]
** `{orch-cli}`. See link:https://docs.openshift.com/container-platform/4.10/cli_reference/openshift_cli/getting-started-cli.html#installing-openshift-cli[Installing the OpenShift CLI].
endif::[]
** link:https://www.gnu.org/software/coreutils/base64[`base64`]. For Mac users, `base64` is available, as part of `coreutils`, on link:https://formulae.brew.sh/formula/coreutils[homebrew].
