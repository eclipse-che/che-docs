...
+
----
pass:c,a,q[{prod-url}]#__<git_repository_url>__ # <1>
----
<1> `__<git_repository_url>__` supports both the HTTPS and SSH protocols, so you can use `pass:c,a,q[https://]` or `git@` like when cloning a remote Git repository.
+
[TIP]
====
