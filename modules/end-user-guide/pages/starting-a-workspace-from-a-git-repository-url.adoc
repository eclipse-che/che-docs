...
By `Git repository URL`, we mean a git clone URL of a repository. Usually, this URL can be fetched by clicking on the `Clone` button in the SCM repository's main page.
...
[NOTE]
====
As an experimental feature, we support a repository url of a directory with a devfile or a repository url of the devfile.
The devfile has to be named as `devfile.yaml` or `.devfile.yaml`.
This feature works with GitHub urls only.
====
