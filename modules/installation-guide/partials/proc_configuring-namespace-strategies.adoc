...
[WARNING]
====
By {kubernetes} definition, the maximum length of namespace name is limited to 63 characters, including the evaluated placeholders.

On OpenShift with multi-host server exposure strategy, the length is further limited to 49 characters.

Be aware that `_<userid>_` placeholder is evaluated into 36 character long UUID string.
====
