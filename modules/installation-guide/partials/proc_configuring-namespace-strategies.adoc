...
ifeval::["{project-context}" == "che"]
With **Helm** installer, {platforms-namespace} strategies are configured using `global.cheWorkspacesNamespace` property.
...
WARNING: The namespace strategies other than per user are deprecated and are subject to removal.
