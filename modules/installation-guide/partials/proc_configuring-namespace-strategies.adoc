...
`self-provisioner` cluster role should be linked to  `system:authenticated:oauth` group.

WARNING: In situations when  `che` ServiceAccount has not enough permissions creating new {orch-namespace} or with OpenShift OAuth
when `self-provisioner` cluster role isn't linked to  `system:authenticated:oauth` group, or in other situations when {prod-short} is not
able or undesirable to create namespaces -  <<pre-creating-namespace>> has to be used.
