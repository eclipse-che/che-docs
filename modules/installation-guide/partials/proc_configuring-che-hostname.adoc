...
+
For logging in to {identity-provider}, follow the xref:setting-up-the-keycloak-{prod-id-short}-username-readonly-theme-for-the-{prod-id}-login-page.adoc#proc_logging-in-to-identity-provider_che[Logging in to {identity-provider}] procedure.
