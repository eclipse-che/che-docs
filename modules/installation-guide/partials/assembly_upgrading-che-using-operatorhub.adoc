...
If the approval strategy in the subscription is set to `Automatic`, the upgrade process starts when a new version of the Operator is published.
If the approval strategy in the subscription is set to `Manual`, the update must be manually approved every time the new version of the Operator is published.
