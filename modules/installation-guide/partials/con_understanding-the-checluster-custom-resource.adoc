...
`CheCluster` Custom Resource is a {kubernetes} object. You can configure it by editing the `CheCluster` Custom Resource YAML file. The file contains sections to configure each component: `auth`, `database`, `server`, `storage`.
