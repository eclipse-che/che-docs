...
Turning swap off is required by {platforms-name}. See this issue for more information.
...
It's time to deploy {prod-upstream-name} by running the Ansible playbook command.
...
If you want to access the cluster API from another computer on your network, install {orch-cli}  first.
