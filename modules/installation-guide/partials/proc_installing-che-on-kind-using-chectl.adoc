...
[NOTE]
====
After completing these steps, if you cannot reach {prod-short} from the host machine, see the `kind` cluster documentation or forums. Look for information on how to make an endpoint available outside the `kind` cluster for your system and network configuration.
====
