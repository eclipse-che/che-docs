...
[NOTE]
====
After all the steps above, you might still be unable to reach {prod-short} from the host machine. If you encounter such problem, see `kind` cluster documentation or forums on how to make an endpoint available outside the kind cluster for your system and network configuration.
====
