...
For x{prod-short} installation, kind cluster should have an Ingress backend and a persitent volumes storage backend. If these requirements are met, go directly to xref:installing-che-on-kind.adoc#run-{prod-cli}-on-kind[deploying {prod-short}].
