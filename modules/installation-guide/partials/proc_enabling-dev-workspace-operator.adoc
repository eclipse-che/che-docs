...
[WARNING]
====
{prod-cli} will automatically setup Dex as OIDC provider on Minikube. For other {kubernetes} clusters, setup OIDC provider following cluster provider documentation and set the following values in {prod-checluster} Custom Resource (CR):
