...
. Check that the entries in the `ca-certs` ConfigMap contain all the additional entries you added in the certificate ConfigMap at the {prod-short} installation level, in addition to the reserved `ca-bundle.crt` entry:
+
[subs="+attributes,+quotes",options="nowrap",role=white-space-pre]
----
...
. Verify that the `ca-certs` ConfigMap has been added as a volume in the workspace Pod:
+
[subs="+attributes,+quotes",options="nowrap",role=white-space-pre]
----
...
. Inspect `/public-certs` folder on the Che-Theia IDE container and check that its contents match the list of entries in the `ca-certs` ConfigMap:
+
[subs="+attributes,+quotes",options="nowrap",role=white-space-pre]
----
