...
Signle-host strategy have two subtypes with different implementation methods. First subtype is named `native`. This strategy is available on Kubernetes, but not on OpenShift, since it uses ingresses for servers exposing. The second subtype named `gateway`, works both on
Kubernetes and OpenShift platforms, and uses a special pod with gateway software running inside to route requests.
