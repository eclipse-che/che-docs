...
- Check that the entries in the `ca-certs` config map contain all the additional entries you added in the certificate config map at the {prod-short} installation level, in addition to the the `ca-bundl.crt` entry which is a reserved one:
+
[subs="+attributes,+quotes",options="nowrap",role=white-space-pre]
----
...
- Inspect `/public-certs` folder on the Theia IDE container and check that its contents match the list of entries in the `ca-certs` config map:
+
[subs="+attributes,+quotes",options="nowrap",role=white-space-pre]
----
