...
| Devfile registry that store descriptions of the sample projects and how to load them as workspaces via the Getting Started tab in the dashboard. See xref:building-an-offline-devfile-registry-image_{context}[].
|===
...
Many workspace plug-ins are run in sidecar containers to ensure their dependencies are available. The offline plug-in registry (see xref:building-an-offline-plugin-registry-image_{context}[]) includes a file with the list of all images referenced in its plug-ins.
