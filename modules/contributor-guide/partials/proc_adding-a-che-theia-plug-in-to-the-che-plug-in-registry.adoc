...
. Submit a PR with the `meta.yaml` file of your Che-Theia plug-in in the `v3/plugins/<your-plugin-publisher-name>/<your-plugin-name>/<version>/` directory to the {prod-short} plug-in registry GitHub repository at link:https://github.com/eclipse/che-plugin-registry/[che-plugin-registry].
+
