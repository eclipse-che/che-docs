...
. Clone the che-plugin-registry repository and check out the version to deploy:
+

include::administration-guide:example$snip_{project-context}-clone-the-plug-in-registry-repository.adoc[]

. Edit the `che-editors.yaml` file.
...
. Navigate to the `./{che-plugin-registry-directory}/v3/plugins/eclipse/che-theia/next` directory. Publish the `meta.yaml` file in this directory to a publicly accessible location where it can be used as an HTTP resource.

. Create a workspace using the sample https://github.com/che-samples/che-theia-branding-example/blob/master/devfile.yaml[che-theia-branding-example devfile] to apply the changes. Make sure the `reference` field points to your published `meta.yaml` file:
+
