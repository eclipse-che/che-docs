...
:description: Adjust workspace storage capacity
:keywords: administration guide, configuring, {prod-id-short}, storage sizes, PVC size, pvc
:navtitle: Adjust workspace storage capacity
...
Adjust the persistent volume claim (PVC) size for workspaces to allocate more or less disk space depending on project requirements and available cluster storage.
...
. Set the appropriate `claimSize` field for the required storage strategy in the Che Cluster Custom Resource.
...
[IMPORTANT]
====
Manually modifying a PVC on the cluster that was provisioned by {prod-short} is not officially supported and might result in unexpected consequences.
