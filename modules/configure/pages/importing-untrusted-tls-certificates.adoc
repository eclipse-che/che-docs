...
:description: Trust custom TLS certificates for external services
:keywords: administration guide, tls, certificate
:navtitle: Trust custom TLS certificates for external services
...
[NOTE]
====
Configure the `CheCluster` Custom Resource to disable the mounting of the CA bundle under the path `/etc/pki/ca-trust/extracted/pem`. Certificates will be mounted under the path `/public-certs` in this case.
[source,yaml]
----
spec:
  devEnvironments:
