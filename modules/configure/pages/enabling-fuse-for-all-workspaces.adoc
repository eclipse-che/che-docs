...
:description: Enable fuse-overlayfs for all workspaces
:keywords: administration-guide, enable, fuse, all, workspaces
:navtitle: Enable fuse-overlayfs for all workspaces
:page-aliases: 
...
Enable the fuse-overlayfs storage driver for all workspaces on the cluster by adding the required pod annotation to the CheCluster Custom Resource.


.Prerequisites

* The xref:enabling-access-to-dev-fuse-for-openshift.adoc[] section has been completed. This is not required for OpenShift versions 4.15 and later.
