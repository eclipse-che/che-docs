...
:description: Configure backup with a regular OCI-compatible registry
:keywords: backup, storage, oci, registry, quay
:navtitle: Configure backup with a regular OCI-compatible registry
//:page-aliases:
...
* Credentials (username and token) for an OCI-compatible registry such as link:https://quay.io[Quay.io] with push permissions to the target repository path.
