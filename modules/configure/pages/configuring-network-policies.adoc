...
:description: Restrict traffic between workspace Pods in different user namespaces by configuring network policies for multitenant isolation.
:keywords: administration guide, configuring, namespace, network policy, network policies, multitenant isolation
:navtitle: Restrict network traffic between workspaces
...
Restrict network traffic between workspace Pods. By default, all Pods in a {orch-name} cluster can communicate with each other even if they are in different namespaces.
