...
:description: Use storage classes to bind persistent volumes from a non-default provisioner.
:keywords: administration guide, configuring, {prod-id-short}, storage class, storage classes
:navtitle: Use a custom storage provisioner
...
Use a custom storage class to bind workspace persistent volumes to specific infrastructure storage, install {prod-short} using storage classes. This is especially useful when you want to bind a persistent volume provided by a non-default provisioner.
...
* A `StorageClass` resource already provisioned in the cluster. You need the exact name of the storage class to configure.
