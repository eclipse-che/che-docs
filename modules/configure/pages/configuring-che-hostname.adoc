...
:description: Use a custom hostname for the dashboard
:keywords: administration guide, configuring, {prod}, {prod-short}, hostname
:navtitle: Use a custom hostname for the dashboard
...
Use a custom hostname to override the auto-detected dashboard URL with a hostname that matches your organization's DNS and TLS certificate.
...
[IMPORTANT]
====
To generate the pair of a private key and certificate, the same certification authority (CA) must be used as for other {prod-short} hosts.
====
