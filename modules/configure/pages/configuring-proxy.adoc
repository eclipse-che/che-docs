...
:description: Route traffic through a proxy by creating a Secret for proxy credentials and setting the proxy configuration in the CheCluster custom resource.
:keywords: administration guide, proxy, networking
:navtitle: Route traffic through a proxy
:page-aliases:
...
Route {prod-short} traffic through a corporate proxy when direct internet access is restricted. Create a proxy credentials Secret and configure the CheCluster Custom Resource.
