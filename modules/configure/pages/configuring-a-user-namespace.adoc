...
:description: Synchronize resources across user namespaces
:keywords: administration guide, configuring, user, namespace
:navtitle: Synchronize resources across user namespaces
:page-aliases: administration-guide:configuring-a-user-namespace.adoc

[id="configuring-a-user-namespace"]
= Synchronize resources across user namespaces


Synchronize ConfigMaps, Secrets, and PVCs from a central {orch-namespace} to every user {orch-namespace}, so developers automatically receive shared resources.
...
* The {kubernetes} resources (ConfigMaps, Secrets, or PVCs) that you want to distribute to all user namespaces are prepared.
...
[NOTE]
====
When both annotations `controller.devfile.io/mount-to-devworkspace-include`
...
+
For other labels and annotations, see link:https://github.com/devfile/devworkspace-operator/blob/main/docs/additional-configuration.adoc#automatically-mounting-volumes-configmaps-and-secrets[Mounting volumes, configmaps, and secrets].
. To use the OpenShift Kubernetes Engine, you can create a `Template` object to replicate all resources defined within the template across each user {orch-namespace}.
+
