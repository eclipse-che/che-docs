...
:description: On {ocp}, collect Eclipse Che and DevWorkspace diagnostic data with must-gather for support and troubleshooting.
...
[role="_abstract"]
On {ocp} clusters, you can collect a diagnostic snapshot of the {prod-short} Operator, {devworkspace}-based Cloud Development Environments, and related cluster state by using the {prod-short} must-gather image with `oc adm must-gather`. Share this dump with others so they can help troubleshoot the issue.
