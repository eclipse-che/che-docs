...
----
+
({prod-short} Operator will set up this password for the backup repository that {prod-short} Operator will create from this custom object on the external backup server.)
+
include::snip_warning-about-losing-backup-repo-password.adoc[]

. Create a secret (for example, `name: ssh-key-secret`) containing the `ssh-privatekey` key with a Base64 encoded private SSH key for logging in to the SFTP server without a password.
