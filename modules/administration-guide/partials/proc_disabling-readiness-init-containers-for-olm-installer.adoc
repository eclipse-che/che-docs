...
= Disabling readiness init containers for OLM installer

To disable readiness init containers for {prod-short} installed using the Operator Lifecycle Manager (OLM) installer:
...
----
. Remove the following environment variable to (from) the Operator Deployment spec:
+
