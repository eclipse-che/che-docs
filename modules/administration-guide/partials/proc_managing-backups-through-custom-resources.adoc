...
The following section describes how to create backups of {prod-short} installation and make a recovery directly through Custom Resource objects.
...
Suppose a user does not want to configure a backup server and agrees to the internal backup server limitations. In that case, it is possible to request automatic configuration from {prod-short} Operator using the `{prod-cli} server:backup` command with no arguments. In such a case, the preparation mentioned above is not required.

. Configure the automatic setup and sending of a backup to the internal backup server:
+
[source,yaml,subs="+attributes"]
----
apiVersion: org.eclipse.che/v1
...
<1> Name of `CheBackupServerConfiguration` object that defines what backup server to use.
...
<2> Current phase of backing up process in the human-readable format.
...
<2> Current phase of the recovery process in the human-readable format.
