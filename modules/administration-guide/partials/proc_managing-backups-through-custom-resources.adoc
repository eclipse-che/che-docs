...
Suppose a user does not want to configure a backup server and agrees to the internal backup server limitations. In that case, it is possible to request automatic configuration from {prod-short} Operator and the preparation mentioned above is not required.
