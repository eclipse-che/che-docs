...
[NOTE]
====
The approach described in this chapter can not be used to recover to a different version of {prod-short}. To recover {prod-short} to another version, use the `{prod-cli}` tool. See the xref:managing-backups-using-chectl.adoc#restoring-from-a-backup-using-chectl_managing-backups-using-{prod-cli}[Restoring {prod-short} from a backup] chapter for more information.
====
