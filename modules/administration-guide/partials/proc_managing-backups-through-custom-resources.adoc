...
[NOTE]
====
The approach described in this chaper can not be used to recover to a different version of {prod-short}. To recover {prod-short} to another version, use the `{prod-cli}` tool. See the xref:managing-backups-using-cli.adoc#cli-restoring-from-backup[Restoring {prod-short} from a backup]) chapter for more information.
====
