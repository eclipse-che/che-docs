...
* A request to create a new backup starts by creating the`CheClusterBackup` object.
...
+
In a case of errors occurrences in your browser after the recovery, clean up the browser data for {prod-chort} domain.
...
Check a recovery process state by reading of the `status` section of the `CheClusterRestore` object:
+
[source,yaml,subs="+attributes"]
----
status:
