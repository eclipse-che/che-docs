...
[subs="+quotes,+attributes"]
----
{orch-cli} wait --for=condition=ready pod -l app.kubernetes.io/component={prod-operator} -n openshift-operators --timeout=120s
----
...
[subs="+quotes,+attributes"]
----
{orch-cli} wait --for=condition=ready pod -l app.kubernetes.io/component={prod-deployment} -n {prod-namespace} --timeout=240s
----
