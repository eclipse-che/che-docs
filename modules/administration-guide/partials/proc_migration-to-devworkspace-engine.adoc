...
run() {
  ...
  for USER_ID in "\${ALL_USERS_IDS[@]}"; do
      ...
      if [ -n "\${IDENTITY_PROVIDER}" ]; then
        ...
        echo "\${USER_ID} \${OPENSHIFT_USER_ID} username:\"\${USER_NAME}\" email:\"\${USER_EMAIL}\" firstName:\"\${USER_FIRST_NAME}\" lastName:\"\${USER_LAST_NAME}\"" >> \${ALL_USERS_DUMP}
      fi
  done
}
...
do
  IDS=($line)
  OPENSHIFT_USER_ID=${IDS[1]}
  USER_EMAIL=$(echo ${IDS[3]} | cut -d ":" -f 2)
  ...
  {orch-cli} exec $POSTGRES_POD -n {prod-namespace}  -- bash  -c "psql ${CHE_POSTGRES_DB} -tAc \"UPDATE usr SET email = '${USER_EMAIL}' WHERE id='${OPENSHIFT_USER_ID}';\""
  {orch-cli} exec $POSTGRES_POD -n {prod-namespace}  -- bash  -c "psql ${CHE_POSTGRES_DB} -tAc \"INSERT INTO profile_attributes(user_id,value, name) VALUES ('${OPENSHIFT_USER_ID}', 'email', '${USER_EMAIL}');\""
