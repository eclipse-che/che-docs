...
[IMPORTANT]
====
You can manage Visual Studio Code extensions by setting up an internal, on-premises Open VSX Registry. This approach provides full control over the extension lifecycle, enables offline use, and improves compliance. The embedded plugin registry is deprecated; the Open VSX Registry is its successor. See xref:running-the-open-vsx-on-premises.adoc[Running the Open VSX Registry on premises] for detailed setup instructions.
====
