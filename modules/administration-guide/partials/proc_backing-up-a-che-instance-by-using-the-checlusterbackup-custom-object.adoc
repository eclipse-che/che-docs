[id="backing-up-a-{prod-id-short}-instance-by-using-the-checlusterbackup-custom-object_{context}"]
= Backing up a {prod-short} instance by using the CheClusterBackup custom object
...
<2> Configures the Operator via this custom resource to use the {prod-short}-managed internal backup server or an administrator-managed external backup server (SFTP, Amazon S3 or S3 API compatible storage, or REST).
+
[TIP]
====
