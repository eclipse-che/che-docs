...
All authentication types use the link:https://jwt.io/introduction/[JWT open standard] as a container for transferring user identity information. In addition, {prod-short} server authentication is based on the link:https://openid.net/connect/[OpenID Connect] protocol implementation, which is provided by default by {identity-provider-url}[{identity-provider2}].
