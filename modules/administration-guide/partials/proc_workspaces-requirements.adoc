...
. Identify the workspace components explicitly specified in the `components` section of the xref:end-user-guide:customizing-workspaces-components.adoc[].
...
See also xref:administration-guide:advanced-configuration-options-for-the-che-server-component.adoc[].
...
* xref:administration-guide:architecture-overview.adoc[].
