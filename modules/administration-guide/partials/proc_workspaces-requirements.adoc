...
.. Default values:
+
The following table displays the default requirements for all workspace components, and the corresponding {prod-short} server properties to modify the defaults cluster-wide.
+
...
+
If the `memoryLimit` and `memoryRequest` attributes of the `containers` section of the `meta.yaml` file are defined, they configure the memory limit of the `chePlugins` or `cheEditors` components. {prod-short} automatically sets the memory request to match the memory limit in case it is not specified explicitly.
+
...
+
If the `memoryLimit` and `memoryRequest` attributes of the devfile are defined, they configure the memory limit of a `dockerimage` container. {prod-short} automatically sets the memory request to match the memory limit in case it is not specified explicitly.
+
[source,yaml]
----
  - alias: maven
