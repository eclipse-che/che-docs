...
[IMPORTANT]
====
The embedded plugin registry is deprecated. Setting up an internal, on-premises Open VSX registry provides full control over the extension lifecycle, enables offline use, and improves compliance. Refer to the xref:proc_deploy-open-vsx-from-source.adoc[] procedure for detailed setup instructions.
====
