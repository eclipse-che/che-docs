...
This section describes how to add Visual Studio Code extensions that are not in the link:https://open-vsx.org/[Open VSX Registry] to {prod-short}'s embedded Open VSX Registry.
...
If you need the extension and the owner cannot or will not publish it, you might be able to fork it and publish it yourself. Consult the license terms of the extension to find out if this is possible. 

You can also import the extension into the local registry from the procedure in this document. 

.Prerequisites
. The Visual Studio Code extension file (.vsix) must be available to download from a valid URL.
