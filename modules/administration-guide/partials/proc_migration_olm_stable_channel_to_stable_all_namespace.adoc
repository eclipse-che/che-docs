...
Delete OLM subscription "{prod-checluster}":
...
Delete current cluster service version by name:

[subs="+quotes,+attributes"]
----
$ CSV_VERSION_NAME=$(oc get clusterserviceversion -n {prod-namespace} -o jsonpath="{.status.currentCSV}")
