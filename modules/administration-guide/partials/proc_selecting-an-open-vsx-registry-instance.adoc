...
* To select the embedded Open VSX registry instance in the `plugin-registry` pod, use `openVSXURL: ''`. See xref:proc_adding-or-removing-extensions-in-the-embedded-open-vsx-registry-instance[this section] for how to customize the list of included extensions.
