...
WARNING: Subscription updates for the {prod-short} Operator revert any customizations of the `CheCluster` custom resource to the published settings for the updated operator. After the update is complete, you must reapply your changes to the `CheCluster` custom resource. You may also choose to build a new registry container based on the updated {prod-short} container, in order to get the latest security fixes and updates.
