...
Note that after a subscription update, your CheCluster will revert to default settings and any changes will need to be reapplied. 
...
[TIP]
====
If the extension author is not cooperative, you can add the extension directly from the `.vsix` file, as long as the license and terms of use allow it, and the file can be accessed from a https:// URL while building the plugin registry container.
