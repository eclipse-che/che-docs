...
NOTE: The `CheClusterRestore` custom object is unusable for recovering a {prod-short} instance of a different version of {prod-short}. To recover a {prod-short} instance of an earlier version of {prod-short} by using `{prod-cli}`, see xref:restoring-a-backed-up-instance-of-an-earlier-version-of-{prod-id-short}_{context}[].
...
. Read the `status` section of the `CheClusterRestore` object to verify the recovery process, for example:
+
