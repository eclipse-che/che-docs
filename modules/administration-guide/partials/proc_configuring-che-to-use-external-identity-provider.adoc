...
+
This requires setting up an OpenShift identity provider ins {identity-provider}. When using an external {identity-provider}, configure the identity provider manually. For instructions, see the appropriate {identity-provider} documentations for either link:{link-identity-provider-documentation-openshift-3}[OpenShift 3] or link:{link-identity-provider-documentation-openshift-4}[OpenShift 4].
