...
.Prerequisites
* xref:assembly_configuring-oauth-1-for-bitbucket.adoc#setting-up-the-bitbucket-application-link_{context}[] is completed.
* The following Base64-encoded files, which were created when xref:assembly_configuring-oauth-1-for-bitbucket.adoc#setting-up-the-bitbucket-application-link_{context}[], are prepared:
