...
patchPreMigrationCheCluster() {
    ...
    FIELDS_2_CLEAN_UP=(/spec/server/dashboardMemoryLimit
    ...
    for FIELD in "${FIELDS_2_CLEAN_UP[@]}"; do
      VALUE=$("${K8S_CLI}" get checluster/"${PRE_MIGRATION_PRODUCT_CHE_CLUSTER_CR_NAME}" -n "${INSTALLATION_NAMESPACE}" -o "jsonpath={$(echo ${FIELD} | tr '/' '.')}")
      if [[ ${VALUE} == "null" ]] || [[ -z ${VALUE} ]]; then
        "${K8S_CLI}" patch checluster/"${PRE_MIGRATION_PRODUCT_CHE_CLUSTER_CR_NAME}" -n "${INSTALLATION_NAMESPACE}" --type=json -p \
                '[{"op": "replace", "path": "'${FIELD}'", "value": "0"}]'
      fi
    done
}
