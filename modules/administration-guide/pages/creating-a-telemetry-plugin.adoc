...
Create a config map referencing the new `plugin.yaml` file.
...
Create a deployment, a service, and a route to expose the web server. The deployment references this config map and places it in the `/var/www/html` directory.
