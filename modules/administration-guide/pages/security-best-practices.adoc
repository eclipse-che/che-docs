...
Network policies provide network-level isolation between {namespace} and pods in {kubernetes}.
By default, {prod-short} requires specific network policies to ensure proper communication between components
while maintaining security boundaries.

The following network policies are essential for {prod-short} operation:

* Allow traffic from OpenShift API server to DevWorkspace webhook server. This policy enables the OpenShift API server to communicate with the DevWorkspace webhook server in the {prod-short} namespace.
+
...
* Allow traffic from workspace namespaces to {prod-short} namespace.
This policy permits pods in workspace namespaces to communicate with services in the {prod-short}
namespace.
+
[source,yaml,subs="+quotes,attributes"]
----
...
* Allow traffic from {prod-short} namespace to user workspaces.
This policy is configured as a template and applied to user workspace namespaces
to allow ingress traffic from the {prod-short} namespace.
+
[source,yaml,subs="+quotes,attributes"]
----
kind: Template
...
metadata:
  name: {prod-id-short}-user-namespace-network-policy-configurator
  namespace: {prod-namespace}
  labels:
    ...
    spec:
      ingress:
        - from:
            - namespaceSelector:
                matchLabels:
                  kubernetes.io/metadata.name: {prod-namespace}
      podSelector: {}
      policyTypes:
        - Ingress
----
