...
. Update a `{prod-id}` client to set {prod-short} redirect URI and web origin:
+
Obtain {prod} URL with the following command:
+
[source,shell,subs="+quotes,+attributes"]
...
EOF
----
<1> Users must have the `self-provisioner` role, in order to create a {platforms-namespace}.
