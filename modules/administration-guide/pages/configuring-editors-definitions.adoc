...
This procedure describes how to configure {prod-short} editors definitions.
...
. Create the `my-editor-definition-devfile.yaml` YAML file with editor definition configuration. Ensure to provide the actual values for `publisher` and `version` under `metadata.attributes`. Below you can find the supported values, including optional ones:
+
