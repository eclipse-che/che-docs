...
configure it to use {identity-provider} as OIDC identity provider.
...
* `cert-manager` installed. See link:https://cert-manager.io/docs/installation[Installing cert-manager].
...
. Save {identity-provider} CA certificate:
+
[subs="+attributes"]
----
...
. Configure {identity-provider} to create realm, client and user:
+
[subs="+attributes"]
----
...
$ {orch-cli} create configmap keycloak-certs --from-file=keycloak-ca.crt=keycloak-ca.crt -n {prod-namespace}
$ {orch-cli} label configmap keycloak-certs app.kubernetes.io/part-of=che.eclipse.org app.kubernetes.io/component=ca-bundle -n {prod-namespace}
...
EOF
$ {prod-cli} server:deploy --platform k8s --domain $(minikube ip).nip.io --che-operator-cr-patch-yaml che-patch.yaml
----
