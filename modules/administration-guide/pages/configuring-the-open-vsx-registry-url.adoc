...
+
--
* To select the embedded Open VSX registry instance in the `plugin-registry` pod, use `openVSXURL: ''`. You can customize the list of included extensions xref:adding-or-removing-extensions-in-a-workspace.adoc[using a workspace] or xref:adding-or-removing-extensions-on-linux.adoc[using a Linux operating system].
