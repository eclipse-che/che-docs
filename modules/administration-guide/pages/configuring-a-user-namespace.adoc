...
. Create the `ConfigMap` below to replicate into every user namespace.
...
+
.Replicate a ConfigMap into every user namespace:
====
[source,yaml,subs="+attributes,+quotes"]
----
kind: ConfigMap
...
====
+
.Replicate a ConfigMap into every user namespace and automatically mount a `settings.xml` file into every user container by path `/home/user/.m2`:
====
[source,yaml,subs="+attributes,+quotes"]
----
kind: ConfigMap
...
. Create the `Secret` below to replicate into every user namespace.
...
for other possible labels and annotations.
+
.Replicate a Secret into every user namespace:
====
[source,yaml,subs="+attributes,+quotes"]
----
kind: Secret
...
====
+
.Replicate a Secret into every user namespace and automatically mount a `trusted-certificates.crt` file into every user container by path `/etc/pki/ca-trust/source/anchors`:
====
[source,yaml,subs="+attributes,+quotes"]
----
kind: Secret
...
====
+
.Replicate a Secret into every user namespace and automatically mount as environment variables into every user container:
====
[source,yaml,subs="+attributes,+quotes"]
----
kind: Secret
