...
** By default, when a workspace has no projects defined in its devfile (for example, the dashboard's built-in "Empty Workspace"), the editor opens with an empty Explorer. To automatically open the `/projects` folder instead, define the __CHE_OPEN_PROJECTS_ROOT_ON_EMPTY__ environment variable in your devfile.
+
[source,yaml]
----
   env:
     - name: CHE_OPEN_PROJECTS_ROOT_ON_EMPTY
       value: "true"
----
+
[NOTE]
