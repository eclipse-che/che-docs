...
Here are the following Custom Resource Definitions provided by the {devworkspace} Operator.

=== {devworkspace}

The {devworkspace} custom resource contains details about an {prod-short} workspace. Notably, it will contain devfile details and a reference to the editor definition. See <<What is {devworkspace} operator>> for more information.
...
.Global DWOC vs {prod-short}-owned DWOC
...
The `devworkspace-webhook-server` deployment can be configured in the global DWOC:
