...
.The Multicluster Redirector Solution for OpenShift
...
The community-supported version is currently available in the link:https://github.com/redhat-developer/devspaces-multicluster-redirector[GitHub repository].
...
The deployment instructions can be found in the link:https://github.com/redhat-developer/devspaces-multicluster-redirector?tab=readme-ov-file#-deploying-to-openshift[Deploying to OpenShift] documentation section.

.Configuration

The routing configuration uses a `ConfigMap` containing JSON that maps OpenShift groups to {prod} URLs. The redirector watches this file and updates routing tables in real-time without requiring restarts.
