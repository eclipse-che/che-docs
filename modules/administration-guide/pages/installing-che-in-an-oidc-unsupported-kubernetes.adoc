...
----
+
. Set the created `kubeconfig` as current by backingup existing `kubeconfig` in the current user's `.kube` or `$KUBECONFIG` folder and replacing it with the newly created one.
...
+
IMPORTANT: Make sure the user has `vcluster-admin` group assignedav.
+
