...
* Ingress Controller. See link:https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/[Ingress Controllers]
...
EOF
----
. Create ClientID and Client Secret in Keycloak. Access the Keycloak UI using the URL `https://$KEYCLOAK_DOMAIN/auth` and log in with the credentials `admin/admin`. Navigate to `Client` Page and Create Client with the following configurations
+
