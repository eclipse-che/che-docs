:page-aliases: administration-guide:proc_deploy-open-vsx-from-workspace.adoc
:_content-type: PROCEDURE
:description: Build the Open VSX extension registry inside a workspace using devfile tasks
:keywords: administration guide, openvsx, deploy, workspace, devfile
:navtitle: Build the extension registry inside a workspace

[id="proc_deploy-open-vsx-from-workspace"]
= Build the extension registry inside a workspace

[role="_abstract"]
Build an on-premises Open VSX extension registry by using predefined devfile tasks in a {prod-short} workspace. The workspace environment includes all necessary tools and commands defined in the `.devfile.yaml` file of the Open VSX repository.
...
.. Execute: `$ oc login --token=__<USER_TOKEN>__ --server=__<CLUSTER_API_URL>__`
