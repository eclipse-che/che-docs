:page-aliases: administration-guide:proc_deleting-extension-from-postgresql-database.adoc
:_content-type: PROCEDURE
:description: Remove an extension directly from the PostgreSQL database
:keywords: administration guide, openvsx, delete, extension, postgresql, database
:navtitle: Remove an extension directly from the database
...
+
Identify the extension publisher. Replace `__<publisher_name>__` with the actual publisher name:
+
[source,sql,subs="+quotes",options="nowrap"]
----
SELECT id, name FROM namespace WHERE name = '__<publisher_name>__';
----
+
Identify the extension ID. Replace `__<publisher_id>__` and `__<extension_name>__` with the values from the previous query:
+
[source,sql,subs="+quotes",options="nowrap"]
----
SELECT id, name, namespace_id FROM extension WHERE namespace_id = __<publisher_id>__ AND name = '__<extension_name>__';
----
+
