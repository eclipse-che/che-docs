...
:description: {prod-short} is a {kubernetes}-native platform that delivers container-based, reproducible cloud development environments to accelerate developer onboarding and eliminate environment inconsistencies.
:keywords: overview, cloud development environment, CDE, workspace, devfile
:navtitle: {prod-short} cloud development environments
:page-aliases: overview:introduction-to-eclipse-che.adoc, overview:what-is-che.adoc

[id="what-is-{prod-id-short}"]
= {prod-short} cloud development environments

[role="_abstract"]
Evaluate {prod-short} as a cloud development platform that delivers container-based, reproducible workspaces on {platforms-name} to accelerate developer onboarding and eliminate environment inconsistencies.
...
{prod-short} workspaces are {platforms-name} Pods that replicate application runtimes used in production and provide a development layer on top — adding intelligent code completion, debugging, and IDE tools. Workspaces are isolated from one another and manage the lifecycle of their own components.
