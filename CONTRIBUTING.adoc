...
* The relevant tools for all edition, validation and publication workflows are built in the link:https://quay.io/repository/eclipse/che-docs[`che-docs` container].
...
. To navigate the preview, open _Preview_.

. To validate links: open _User Runtimes / che-docs / Validate links_.

. To validate language changes: open _My Workspace / User Runtimes / che-docs / Validate language changes_.

. To validate the language of the files currently open, look at the _Problems_ tab in the _Bottom Panel_. To toggle the view of this tab use the _View > Problems_ menu entry.
...
. Open a terminal into the project directory, and build the documentation using an instance of the `che-docs` container:
+
----
...
. Open a second terminal, and attach it to the running `che-docs` container. Run all the following commands from this context.
+
----
...
. To create a new topic using the modular templates, run:
+
----
$ ./tools/newtopic.sh
----
